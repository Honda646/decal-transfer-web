...
import { HelmetState, HelmetType, StyleId, ActiveTab, FullViewResult } from './types';
import { UploadIcon, TranslateIcon, CopyIcon, XCircleIcon, LockIcon, BackIcon, SparklesIcon } from './icons';
            ...
            >
                ...
                {helmet.previewUrl ? (
                    <div className="preview-wrapper">
                     ...
                     <div className="upload-prompt">
                        <SparklesIcon />
                        <strong>AI will generate a base helmet</strong>
                        <p>A new "{selectedType}" helmet will be created.</p>
                    </div>
                ) : (
                    <div className="upload-prompt">
                        <UploadIcon />
...
export const DecalPromptBuilder = ({
  prompt, proPrompt, onPromptChange, onTranslate, activeLang, activePromptTab, onActivePromptTabChange
}: { 
    prompt: string; 
    proPrompt: string; 
    ...
    onTranslate: () => void; 
    activeLang: 'vi' | 'en';
    activePromptTab: 'simple' | 'pro';
    onActivePromptTabChange: (tab: 'simple' | 'pro') => void;
}) => {
  const highlighterRef = useRef<HTMLDivElement>(null);
  const textareaRef = useRef<HTMLTextAreaElement>(null);

  const handleCopy = () => {
    const textToCopy = activePromptTab === 'simple' ? prompt : proPrompt;
  ...
  return (
    <div className="prompt-builder">
      ...
      <div className="results-tabs prompt-tabs">
        <button 
          className={`tab-button ${activePromptTab === 'simple' ? 'active' : ''}`}
          onClick={() => onActivePromptTabChange('simple')}
        >
          Simple
        </button>
        <button 
          className={`tab-button ${activePromptTab === 'pro' ? 'active' : ''}`}
          onClick={() => onActivePromptTabChange('pro')}
          disabled={!proPrompt}
          title={!proPrompt ? "Analyze Helmet 1 to generate PRO prompt" : ""}
        >
          PRO
        </button>
      </div>
      
      {activePromptTab === 'simple' && (
      ...
      {activePromptTab === 'pro' && proPrompt && (
        <div className="prompt-editor-wrapper">
          <textarea
            className="prompt-editor-textarea--readonly"
            value={proPrompt}
            readOnly
            aria-label="PRO Decal Prompt"
            spellCheck="false"
          />
        </div>
      )}
...
export const ResultsDisplay = ({
    ...
    styleId, onStyleIdChange,
    strength, onStrengthChange,
    // FIX: Alias prop to avoid potential name confusion
    preserveFinish: stylePreserveFinish,
    onPreserveFinishChange: onStylePreserveFinishChange,
    lockSeed: styleLockSeed,
    onLockSeedChange: onStyleLockSeedChange,
    onApplyOrReload,
    isCached
}: {
    result: string | null;
    promptTypeUsed: 'simple' | 'pro' | null;
    isLoading: boolean;
    loadingText: string;
    helmet1Status: 'idle' | 'analyzing' | 'extracted' | 'error';
    onStartOver: () => void;
    activeTab: ActiveTab;
    setActiveTab: (tab: ActiveTab) => void;
    // Download Handlers
    onDownloadSingleView: () => void;
    onDownloadFullView: () => void;
  ...
  return (
    <div className="results-panel">
      ...
      <div className="tab-content">
        {activeTab === 'single' && (
            (isLoading || helmet1Status === 'analyzing') ? <Spinner text={helmet1Status === 'analyzing' ? 'Analyzing Helmet 1...' : loadingText} /> :
            result ? (
                <>
                    <div className="result-image-wrapper"><img src={result} alt="Generated helmet" className="result-image" /></div>
                    <div className="result-actions">
                        {promptTypeUsed && (
                            <div className="prompt-source-chip">
                                Generated with: <strong>{promptTypeUsed.toUpperCase()} PROMPT</strong>
                            </div>
                        )}
        ...
        {activeTab === 'full-view' && (
            <>
                ...
                fullViewResult ? (
                    <>
                        <div className="result-image-wrapper"><img src={fullViewResult} alt="Full view composite" className="result-image" /></div>
                        <div className="result-actions">
                             <button className="download-button" onClick={onDownloadFullView}>Download</button>
                        </div>
                    </>
                ) : (
                     <p className="placeholder-text">Click 'Generate Full View' to create a multi-angle product shot of your helmet.</p>
                )}
                
                {result && !isGeneratingFullView && (
                   <div className="panel-view">
                     <div className="panel-actions">
                         <button className="cta-button" onClick={onGenerateFullView} disabled={isGeneratingFullView}>
                            {fullViewResult ? 'Regenerate Full View' : 'Generate Full View'}
                         </button>
                     </div>
                   </div>
               )}
            ...
            <>
                {isStyling ? <Spinner text={loadingText} /> :
                 stylePackResult ? (
                    ...
                    <p className="placeholder-text">Select a style, adjust the settings, and click 'Apply Style' to transform your result.</p>
                 )}
                 {result && (
// FIX: Explicitly pass children to PanelView to resolve a potential JSX parsing issue.
                    <PanelView 
                        title="Style Pack Controls" 
                        ...
                        children={
                            <StylePackView
                                ...
                                preserveFinish={stylePreserveFinish} onPreserveFinishChange={onStylePreserveFinishChange}
                                lockSeed={styleLockSeed} onLockSeedChange={onStyleLockSeedChange}
